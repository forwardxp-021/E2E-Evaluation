...
def run(args):
    ...
    for m in eval_methods:
        ...
        sp={k:float(spearmanr(dist(methods[m][ev],methods[m][ev],args.distance)[np.triu_indices(len(ev),1)],np.abs(sig[k][ev][:,None]-sig[k][ev][None,:])[np.triu_indices(len(ev),1)]).correlation) for k in CORR_KEYS}
