...
def run(args):
    ...
    if ps:
        pline = f"- n_total: {ps.get('n_total')}\n- n_labeled: {ps.get('n_labeled')}\n- n_unlabeled: {ps.get('n_unlabeled')}"
    else:
        pline = 'WARNING: pseudo_label_summary.json missing.'
    if learned_evaluated:
        next_steps = '- Analyze learned vs baseline trade-offs.\n- Improve jerk/comfort sensitivity.\n- Add qualitative retrieval examples.\n- Generate paper-ready tables.\n- Prepare ablation of jerk-aware loss.'
    else:
        next_steps = '- Train/export row-level learned embedding.'
