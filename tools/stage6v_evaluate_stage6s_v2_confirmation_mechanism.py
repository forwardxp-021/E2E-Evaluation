#!/usr/bin/env python3
"""Locked Stage6S-v2 confirmation mechanism gate; never reads embeddings."""
...
from tools.stage6s_v2_evaluate_development_mechanism import (
    PLANNERS, directional_fraction, finite, mean, median, metrics,
)
...
def bootstrap(pairs: list[dict[str, Any]], design: dict[str, Any]) -> dict[str, Any]:
    ...
    metric_keys = [
        "delta_mean_speed", "delta_rms_accel", "delta_median_front_gap", "delta_median_finite_thw",
        "delta_mean_accel_during_closing", "delta_mean_accel_during_following_pressure",
    ]
    samples = {key: np.empty(reps, dtype=float) for key in metric_keys}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    auth = read_json(AUTH)
    if auth.get("stage6s_v2_representation_evaluation_condition") != "ONLY_IF_CONFIRMATION_MECHANISM_GATE_PASSES":
        raise ValueError("conditional embedding rule changed")
    design = read_json(args.design)
    freeze = read_json(args.freeze_manifest)
    view = read_json(args.view_dir / "stage6s_v2_confirmation_view_summary.json")
    ...
    if view.get("status") != "STAGE6S_V2_CONFIRMATION_VIEW_READY" or view.get("scenario_count") != expected:
        raise ValueError("confirmation view incomplete")
    if view.get("embedding_or_bdd_read") is not False or validation.get("pass") is not True:
        raise ValueError("confirmation view/context validation failed")
    meta = read_csv(args.context_dir / "metadata.csv")
    ledger = read_csv(args.view_dir / "stage6s_v2_confirmation_scenario_ledger.csv")
    ...
    if args.output_dir.exists():
        if not args.overwrite: raise FileExistsError(args.output_dir)
        shutil.rmtree(args.output_dir)
    args.output_dir.mkdir(parents=True)
    with (args.output_dir / "stage6s_v2_confirmation_pair_mechanisms.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    result = safe({
        "schema_version": "stage6v_stage6s_v2_confirmation_mechanism_v1",
        "status": "CONFIRMATION_MECHANISM_GATE_PASS_REPRESENTATION_EVALUATION_AUTHORIZED" if passed else "CONFIRMATION_MECHANISM_GATE_FAILED_STOP_NO_EMBEDDING",
        "mechanism_gate_passed": passed, "complete_pairs": len(pairs), "distinct_logs": len(set(r["log_name"] for r in pairs)),
        "aggregate": aggregate, "directional_pair_fractions": directional,
        "interaction_checks": {k: bool(v) for k,v in interactions.items()}, "gate_checks": {k: bool(v) for k,v in checks.items()},
        "log_cluster_bootstrap": bootstrap(pairs, design), "thw_definition": design["thw_definition"],
        "embedding_or_bdd_read": False, "training_or_protocol_modified": False,
        "authorization_sha256": AUTH_SHA, "design_sha256": sha256(args.design), "freeze_manifest_sha256": sha256(args.freeze_manifest),
    })
    (args.output_dir / "stage6s_v2_confirmation_mechanism_summary.json").write_text(json.dumps(result, indent=2, ensure_ascii=False, allow_nan=False)+"\n", encoding="utf-8")
    return result
