...
def run(args):
    ...
    plt.figure(figsize=(6,4)); plt.bar(df['run'], df['centroid_accuracy_overall']); plt.tight_layout(); plt.savefig(out/'comparison_classification_bar.png'); plt.close()
    plt.figure(figsize=(6,4)); plt.bar(df['run'], df['hit_at_1']); plt.tight_layout(); plt.savefig(out/'comparison_retrieval_bar.png'); plt.close()
    ...
    plt.xticks(x+1.5*w, df['run']); plt.legend(fontsize=8); plt.tight_layout(); plt.savefig(out/'comparison_style_correlation_bar.png'); plt.close()
