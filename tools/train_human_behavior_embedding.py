...
def sanitize_trajectory_array(traj, *, mode="interpolate", max_nan_ratio=0.2):
    arr = _to_dense_traj(traj)
    n, t, d = arr.shape
...
def normalize_local(tr):
    ...
    v0 = v[torch.arange(v.shape[0]), first_valid_idx]
    disp = xy[:, min(5, xy.shape[1] - 1), :] - xy[:, 0, :]
    use_disp = (~has_valid_v) | (torch.linalg.norm(v0, dim=-1) <= 1e-6)
    heading_vec = torch.where(use_disp[:, None], disp, v0)
    bad_h = torch.linalg.norm(heading_vec, dim=-1) <= 1e-6
    heading_vec[bad_h] = torch.tensor([1.0, 0.0], device=tr.device)
    ...
    assert torch.isfinite(out).all(), "normalize_local produced non-finite values"
    return out
