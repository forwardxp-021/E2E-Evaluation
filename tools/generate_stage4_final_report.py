...
def _load_comparison(path: Path):
    df = pd.read_csv(path)
    if "run" not in df.columns and "stage" in df.columns:
...
def _parse_aux_metrics(aux_raw):
    if not isinstance(aux_raw, dict):
        return {}, ["Aux metrics JSON root is not an object."]
    warnings = []
    container = aux_raw.get("metrics", aux_raw)
    parsed = {}
    if isinstance(container, dict):
        ...
        for i, item in enumerate(container):
            if not isinstance(item, dict):
                warnings.append(f"Aux metrics list item {i} is not an object.")
                continue
            target = item.get("target")
            if not target:
                warnings.append(f"Aux metrics list item {i} missing target.")
                continue
            parsed[str(target)] = item
    else:
        warnings.append("Aux metrics container is neither dict nor list.")
...
def main(args):
    ...
    aux_path = Path(args.stage4g_aux_json)
    parsed_aux, aux_warnings = _parse_aux_metrics(_read_json(aux_path))
    targets = ["rms_accel", "rms_jerk", "max_abs_accel", "max_abs_jerk", "mean_thw", "min_thw"]
    aux_rows = []
    for t in targets:
        m = parsed_aux.get(t)
        if m is None:
            aux_warnings.append(f"Missing target '{t}' in aux metrics.")
            aux_rows.append({"Target": t, "MAE": "N/A", "RMSE": "N/A", "Spearman": "N/A", "Valid Pairs": "N/A"})
            continue
        row = {
            "Target": t,
            "MAE": _coerce_metric_value(m.get("mae")),
            "RMSE": _coerce_metric_value(m.get("rmse")),
            "Spearman": _coerce_metric_value(m.get("spearman")),
            "Valid Pairs": _coerce_metric_value(m.get("valid_pairs", m.get("n_valid"))),
        }
        for k in ("MAE", "RMSE", "Spearman", "Valid Pairs"):
            if row[k] is None:
                aux_warnings.append(f"Target '{t}' has missing/NaN value for {k}.")
                row[k] = "N/A"
        aux_rows.append(row)
    ...
    for m in sanity_metrics:
        ...
        if m == "spearman_rms_jerk_delta":
            interp = "Shuffling comfort metric target collapses jerk-sensitive geometry, supporting meaningful comfort alignment." if h < g else "Jerk-sensitive geometry did not collapse as expected; inspect shuffled-target pipeline."
        elif m in {"hit_at_1", "centroid_accuracy_overall", "mean_same_label_fraction_topk"}:
            interp = "Shuffling target reduces retrieval/classification gain, supporting that Stage 4G improvement is not a generic metric-loss artifact." if h < g else "Retrieval/classification gain did not drop; verify shuffled-target setup."
        else:
            interp = "Not the primary sanity-check target. Lateral/curvature correlations may remain high due to trajectory geometry; the key sanity result is jerk collapse."
        rows.append({"Metric": label_map[m], "Stage 4G true target": g, "Stage 4H shuffled target": h, "Drop": g - h, "Interpretation": interp})
    ...
    plt.figure(figsize=(8, 4)); plt.bar(stages, jerk_vals, color=colors); plt.xticks(rotation=20); plt.ylabel("rms_jerk_delta"); plt.title("Stage 4 jerk-sensitive geometry: Stage 4G succeeds, Stage 4H sanity check collapses")
    stage4d_ref = _metric(comp_df, "stage4d_v1", "spearman_rms_jerk_delta")
    if not pd.isna(stage4d_ref):
        plt.axhline(stage4d_ref, linestyle="--", linewidth=1.0, color="gray", label="Stage 4D reference")
        plt.legend()
    plt.tight_layout(); plt.savefig(out_dir / "figure_stage4_jerk_delta.png", dpi=180); plt.close()
    ...
    plt.figure(figsize=(8, 4)); sanity_plot = ["spearman_rms_jerk_delta", "hit_at_1", "centroid_accuracy_overall"]; xp = np.arange(3); wg = 0.35
    ...
    warnings_block = "\n".join([f"- WARNING: {w}" for w in aux_warnings]) if aux_warnings else "- WARNING: none"
...
Stage 4H is not expected to reduce every correlation. Its purpose is to test whether jerk/comfort geometry depends on meaningful comfort targets. The key result is that rms_jerk_delta collapses from Stage 4G to Stage 4H, while Hit@1 and TopK same-label fraction also fall back near earlier stages. This supports Stage 4G as meaningful comfort metric alignment.

The shuffled-target sanity check is designed primarily to test jerk/comfort-sensitive geometry. The main evidence is that rms_jerk_delta drops from Stage 4G to Stage 4H and retrieval gains disappear. Yaw/curvature correlations are secondary and may remain high due to geometric trajectory structure.

## 9. Source files and warnings

- Stage 4G aux metrics source: `{aux_path}`
- Aux metrics parsed successfully: {'yes' if len(parsed_aux) > 0 else 'no'}
{warnings_block}

## 10. Reviewer-facing limitations
...
"""
    ...
    numbers = {"ablation": ab_df.to_dict(orient="records"), "stage4g_learned_vs_baselines": lvb_df.to_dict(orient="records"), "stage4g_aux": aux_df.to_dict(orient="records"), "stage4h_sanity": sanity_df.to_dict(orient="records")}
