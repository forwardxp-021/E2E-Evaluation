...
Stage 6B behavior-event bins were coarse localization bins. Stage 6C upgrades this into dynamic interaction exposure and event-specific style diagnosis. In Stage 6C, `exposure_*` bins are candidates for dynamic matching/control, while `outcome_*` bins remain report/localization variables because they may encode behavior style itself.
