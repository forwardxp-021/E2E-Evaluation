...
#### (d) Within-source retrieval — centroid-agreement hit rate

```
"within_source_retrieval": {
    "metric_mode": "within_source_centroid_agreement",
    "centroid_agreement_hit_rate": ...,   ← 应高于 chance (0.5000 for 3 policies)
    "chance_hit_rate": 0.5,
...
**关于 metric_mode**：
- `within_source_centroid_agreement`：在同 source 内，query policy 的最近邻是否与 centroid 结构预期一致。
  - Chance = 1/(n_policies-1) = 0.5（从 2 个其他 policy 中随机选 1 个）
  - 高于 chance 说明 embedding 的局部排序与全局 centroid 结构一致
- **不使用旧的 "same-policy hit rate"**，因为 within-source 设定下每个 source 只有 1 个样本/policy，
  同 policy 邻居不存在，旧定义恒为 0.0（bug 已修复）。

### 冒烟测试（aligned retrieval 回归测试）
```bash
python scripts/smoke_test_aligned_retrieval.py
```
验证 centroid-agreement hit rate 在结构化数据上高于 chance，且不再出现恒为 0.0 的假象。
...
✅ 修复 `evaluate_policy_separation_aligned.py`：within-source retrieval 指标恒为 0.0 的 bug  
　　→ 原因：within-source 每 policy 只有 1 个样本，"同 policy 最近邻" 定义无效，恒为 False  
　　→ 修复：改为 "centroid-agreement" 定义（最近邻是否与 centroid 结构预期一致），chance=0.5  
　　→ summary.json 新增 `metric_mode` / `metric_definition` 字段，键名改为 `centroid_agreement_hit_rate`  
